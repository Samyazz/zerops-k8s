...
Status: complete; static validation and live staging/production acceptance passed.
...
| [x] | B-05 | B-01 | Generalize the edge proxy backends and make Headlamp/unused listeners optional. `full` keeps current defaults; `production` targets one API backend and two Traefik NodePorts. | Go tests cover disabled routes, one API backend, backend failover, readiness, and full-profile compatibility. |
...
| [x] | S-01 | B-03, B-04, B-06 | Define staging services: only `k8scp1` and `k8sworker1`, Lightweight Core for new imports, locally built node images, no S3 and no edge. | `validate-recipe.sh staging` proves exact topology/resources and zero references to absent services. |
| [x] | S-02 | S-01 | Expose the API as `k8scp1.zerops:6443` over VPN and application ingress through the worker's Traefik NodePort/HTTP port. Set the kubeadm control-plane endpoint per profile. | Kubeconfig reaches `/readyz` through VPN; the ingress smoke URL succeeds; the API port has no public HTTP route. |
...
| [x] | S-05 | S-03 | Enforce “no orbiting services” in validation and cleanup. This includes no edge, backup storage, Grafana/Prometheus/ELK/APM, database, cache, or cluster UI service created by the recipe. | Exact Zerops service check passes while a fixture proves a pre-existing unrelated service is preserved. |
...
| [x] | S-07 | S-02, S-04, S-05, S-06 | Add staging acceptance and evidence collection. | Exact services, two Ready nodes, DNS, pod networking, Gateway traffic, platform service logs/stats, security checks, and stop/start recovery pass. Forbidden-component evidence is included. |
...
| [x] | D-02 | B-03, D-01 | Add first-class recipe publishing notes and direct import links for all three entry points. Explain that profile imports are alternative topologies, not simultaneous clusters. | Links resolve at the pushed commit and recipe descriptions match the exact inventories. |
| [x] | Q-01 | B-01 through W-04 | Add shell/Go/Python tests and fixtures for profile rendering, edge routing, service pruning, forbidden references, workflow capability gates, and evidence redaction. | `go test ./...`, `go vet ./...`, Python tests, `shellcheck scripts/*.sh`, workflow lint, and `git diff --check` pass. |
...
| [x] | L-03 | L-02 | If `FINAL_PROFILE` is not `production`, cleanly switch to the requested final profile and run its acceptance level before handoff. | Not applicable: `FINAL_PROFILE=production`; the L-02 production state is the required final state and was reverified before handoff. |

...
2. `staging` — minimal staging with one control plane, one worker, ingress, and only the required two-instance VRRP/HAProxy edge as an outer runtime.
...
| Stable outer edge | Keepalived VRRP + 2 HAProxy `k8sedge` containers | Keepalived VRRP + 2 HAProxy `k8sedge` containers | Keepalived VRRP + 2 HAProxy `k8sedge` containers |
...
| [x] | B-05 | B-01 | Run HAProxy behind a Keepalived multicast-VRRP VIP in all profiles. Derive host `.222` in the last project `/24`; discover the interface/address at startup and keep static peers out of the configuration. | Render and live election tests cover changing replica IPs, one VIP owner, HAProxy health tracking, and backup takeover. |
...
| [x] | S-02 | S-01 | Expose the API and application ingress through the derived VRRP VIP and HAProxy to the worker's Traefik NodePort. Include the VIP as an API certificate IP SAN. | Kubeconfig reaches `/readyz` with normal IP verification; the ingress smoke URL succeeds; the API port has no public HTTP route. |
...
| [x] | S-05 | S-03 | Enforce the exact minimal inventory. Apart from the required VRRP/HAProxy edge, no backup storage, Grafana/Prometheus/ELK/APM, database, cache, or cluster UI service is created. | Exact Zerops service check passes while a fixture proves a pre-existing unrelated service is preserved. |
...
| [x] | S-07 | S-02, S-04, S-05, S-06 | Add staging acceptance and evidence collection. | Exact services, two Ready nodes, VRRP API/TLS, HAProxy ingress, DNS, pod networking, Gateway traffic, platform service logs/stats, security checks, and stop/start recovery pass. Forbidden-component evidence is included. |

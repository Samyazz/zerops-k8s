...
Status: live revalidation in progress after acceptance and failure-recovery hardening.
...
| [ ] | L-03 | L-02 | If `FINAL_PROFILE` is not `production`, cleanly switch to the requested final profile and run its acceptance level before handoff. | Not applicable when `FINAL_PROFILE=production`; leave production running after L-02 and reverify the final state. |
